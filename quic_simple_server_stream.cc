...
void QuicSimpleServerStream::SendResponse() {
  ...
  if (!QuicContainsKey(request_headers_, ":authority") ||
    ...
    // yyq
    if (QuicContainsKey(request_headers_, ":yyq")) {
        SpdyHeaderBlock headers;
        headers[":status"] = "200";
	headers[":yyq"] = "yyq server response";
	std::string body = "yyq server send body data";			  
        headers["content-length"] = QuicTextUtils::Uint64ToString(body.length());
	SendHeadersAndBody(std::move(headers), body);
    }
    else {
        SendErrorResponse();
    }
